 ...
 *       .tag("My custom tag")   // (Optional) Global tag for every log. Default JackLogger
 *       .build();
 * </code></pre>
 */
public class PrettyFormatStrategy implements FormatStrategy {
  ...
  private int getStackOffset(@NonNull StackTraceElement[] trace) {
    ...
    for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
      ...
      if (!name.equals(LoggerPrinter.class.getName()) && !name.equals(JackLogger.class.getName())) {
        return --i;
      }
    }
    return -1;
  ...
  public static class Builder {
    ...
    @Nullable String tag = "JackLogger";
